...
// interface.
// In order to deduce the device to be used for the routes, we will parse
// the network config data and see, for each route, what device is associated
// to its source address. If we cannot find such a device, we fallback to the
// agent device passed as parameter.
...
// Parse the network configuration and return a map from IP addresses to the
// associated device.
