...
// NodeInterfaceID uniquely identifies a node interface in the connections
// graph.
type NodeInterfaceID string

// NodeInterface keeps node interface related data in the connections graph.
...
// A ConnectionsGraph is an undirected graph keeping the connection information
// between node interfaces.
// Its main usage is for routing purposes: knowing the entire topology
// connections graph allows to build route structures which can be further used
// by callers to set up routes using the lizard REST API.
type ConnectionsGraph struct {
...
// NewNodeInterface creates a new node interface for the connections graph.
...
func NewNodeInterfaceForRange(
	...
	intfValue := nodeRangeValue.FieldByName("Interfaces").FieldByName(strings.Title(intfType))
	...
	gateway := intfValue.FieldByName("ConnectivitySettings").FieldByName("GwStart").Interface().(*string)
...
// NewConnectionsGraph is used to create a new connections graph.
func NewConnectionsGraph(logger *logging.Logger) *ConnectionsGraph {
...
func (g *ConnectionsGraph) addNodeInterface(id NodeInterfaceID, intf *NodeInterface) {
...
func (g *ConnectionsGraph) addDirectedConnection(srcID, destID NodeInterfaceID) {
...
func (g *ConnectionsGraph) AddConnection(srcIntf, destIntf *NodeInterface) {
	srcID := srcIntf.ID()
	destID := destIntf.ID()
...
func (g *ConnectionsGraph) AddConnectionForRanges(
	srcAgentID string,
...
func (g *ConnectionsGraph) HasConnection(srcIntfID, destIntfID NodeInterfaceID) bool {
...
// NumConnections is used to get the number of connections in the graph.
func (g *ConnectionsGraph) NumConnections() int {
	num := 0
...
func (g *ConnectionsGraph) needsRouteAdded(srcIntf, destIntf *NodeInterface) (bool, error) {
...
// interface.
func (g *ConnectionsGraph) GetRoutes(
	agentID string,
	nodeType mwapi.LCNodeType,
	intfType string,
	device string,
	...
	for intfID := range intfIDs {
		...
		for peerIntfID := range peerIntfIDs {
			...
			needsRoute, err := g.needsRouteAdded(intf, peerIntf)
			if err != nil {
				return nil, err
			}
			if !needsRoute {
...
func (g *ConnectionsGraph) getRoute(
